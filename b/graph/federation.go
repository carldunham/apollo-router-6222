...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Animal":
		resolverName, err := entityResolverNameForAnimal(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Animal": %w`, err)
		}
		switch resolverName {
		...
		case "findAnimalByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findAnimalByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindAnimalByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Animal": %w`, err)
			}
...
func entityResolverNameForAnimal(ctx context.Context, rep EntityRepresentation) (string, error) {
	for {
		var (
		...
		return "findAnimalByID", nil
	}
	return "", fmt.Errorf("%w for Animal", ErrTypeNotFound)
}
